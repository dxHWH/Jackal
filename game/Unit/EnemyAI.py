...
class EnemyAI:
    ...
    SEPARATION_WEIGHT = 1.2                  # 分离力权重
    STRAFE_WEIGHT = 0.65                     # 环绕机动权重
    ...
    def update(self, delta_time: float, unit_manager, bullet_manager, game_map):
        ...
        tactical_dir, tactical_state = self._get_tactical_move_direction(target, target_dir)
        prefer_backward = tactical_state == "retreat"

        # 决策步骤 1：检查是否已经与障碍物碰撞
        if self._is_colliding():
            best_dir = self._get_collision_escape_direction(tactical_dir)
            if best_dir is not None:
                self.ai_state = "collision_escape"
                self._move_towards_direction(best_dir, prefer_backward=prefer_backward)
                # 碰撞脱离后仍然尝试瞄准和开火
        ...
        # 决策步骤 2：主动避障（预测未来碰撞）
        avoid_dir = self._get_obstacle_avoidance_direction(tactical_dir)
        if avoid_dir is not None:
            self.ai_state = "avoid_obstacle"
            self._move_towards_direction(avoid_dir, prefer_backward=prefer_backward)
            self._aim_at_target(target)
            self._try_fire(target)
            return

        # 决策步骤 3：子弹规避
        evade_dir = self._get_bullet_evasion_direction(tactical_dir)
        if evade_dir is not None:
            self.ai_state = "evade_bullet"
            self._move_towards_direction(evade_dir, prefer_backward=prefer_backward)
            self._aim_at_target(target)
            self._try_fire(target)
            return

        # 默认行为：根据交战距离进行接近/环绕/后撤，而不是一味硬冲。
        self.ai_state = tactical_state
        self._move_towards_direction(tactical_dir, prefer_backward=prefer_backward)
        self._aim_at_target(target)
        self._try_fire(target)
    ...
    def _get_separation_direction(self) -> Tuple[float, float]:
        """对近邻单位施加分离力，减少坦克之间重叠和顶牛。"""
        ...
        for other in self.unit_manager.units:
            ...
            dx = my_x - other.position[0]
            dy = my_y - other.position[1]
            dist = math.hypot(dx, dy)
            if dist <= 1e-6 or dist >= self.SEPARATION_DISTANCE:
                continue

            weight = (self.SEPARATION_DISTANCE - dist) / self.SEPARATION_DISTANCE
            repel_x += (dx / dist) * weight
            repel_y += (dy / dist) * weight

        if abs(repel_x) <= 1e-6 and abs(repel_y) <= 1e-6:
            return (0.0, 0.0)
        return self._normalize((repel_x, repel_y))

    def _get_tactical_move_direction(
        self,
        target: BaseUnit,
        target_dir: Tuple[float, float],
    ) -> Tuple[Tuple[float, float], str]:
        """
        结合交战距离与分离力输出移动方向，避免 AI 贴脸硬撞。
        返回 (direction, state_name)。
        """
        distance = self._distance_to(target)

        if distance < self.COMBAT_TOO_CLOSE_DISTANCE:
            base_dir = (-target_dir[0], -target_dir[1])
            state = "retreat"
        elif distance > (self.COMBAT_PREFERRED_DISTANCE + self.COMBAT_DISTANCE_TOLERANCE):
            base_dir = target_dir
            state = "approach"
        else:
            # 在理想交战距离附近做侧向环绕，不直接压到目标中心。
            strafe_dir = (-target_dir[1] * self.strafe_sign, target_dir[0] * self.strafe_sign)

            # 轻微径向修正，防止环绕时持续向外漂移。
            if distance >= self.COMBAT_PREFERRED_DISTANCE:
                radial = target_dir
            else:
                radial = (-target_dir[0], -target_dir[1])

            radial_weight = min(
                0.5,
                abs(distance - self.COMBAT_PREFERRED_DISTANCE) / max(self.COMBAT_PREFERRED_DISTANCE, 1.0),
            )
            base_dir = (
                strafe_dir[0] * self.STRAFE_WEIGHT + radial[0] * radial_weight,
                strafe_dir[1] * self.STRAFE_WEIGHT + radial[1] * radial_weight,
            )
            state = "strafe"

        separation_dir = self._get_separation_direction()
        mixed_dir = (
            base_dir[0] + separation_dir[0] * self.SEPARATION_WEIGHT,
            base_dir[1] + separation_dir[1] * self.SEPARATION_WEIGHT,
        )
        return self._normalize(mixed_dir), state
    ...
    # ----------------- 移动控制 -----------------
    def _move_towards_direction(self, desired_dir: Tuple[float, float], prefer_backward: bool = False):
        """
        根据期望方向控制车身转向和前进
        """
        ...
        # 在后撤态下，若目标方向落在车身后半区，直接倒车可更快拉开距离。
        if prefer_backward and abs(angle_diff) > 90.0:
